...
const traverseRouteChain = (msg, ws, chain, errorCallback) => {
    let handled = false;
    ...
    const next = () => {
        if (chain.length > 0) {
            ...
            if (handler.verb === 'all' || handler.verb === ws.verb) {

                try {
                    handled = true;
                    const returned = handler.dispatch(msg, ws, next);
...
const Router = () => {
    ...
    const router = {
        ...
        dispatch: (msg, ws) => {
            const route = trimRoute(ws.route);
            const routes = router.getRoutes()[route];
            const chain = routes ? combineRouteChains([], routes) : [];

            traverseRouteChain(msg, ws, chain, (err) => {
                router.dispatchError(err, ws);
            });
        },

        dispatchError: (err, ws) => {
            const chain = combineRouteChains([], router.getErrorHandlers());

            // No error handler middleware, respond with the error from the route
            if (chain.length === 0) {
                ws.respondWithError(err);
                return;
            }

            traverseRouteChain(err, ws, chain, (errorHandlerError) => {
                ws.respondWithError(errorHandlerError);
            });
        }
    };
