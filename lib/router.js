...
const Router = () => {
    ...
    const addRoute = (route, chain) => {
        ...
        if (!util.isString(route)) {
            throw new Error('Only string routes can be used for routing');
        }
        ...
        if (isRegexRoute(route)) {
            ...
            if (!routes[cleanRoute]) {
                routes[cleanRoute] = { chain: appendRouteChain([], globalHandlers) };
            }
    ...
    const addRouter = (route, router) => {
        ...
        if (!util.isObject(router) || !util.isFunction(router.getRoutes) || !util.isFunction(router.getRegexRoutes) || !util.isFunction(router.getErrorHandlers)) {
            throw new Error('Only Rapport routers can be added to a route with use()');
        }
    ...
    const router = {
         ...
         * @param {...object|...function} router The routers or global handlers to add.
         */
        use: (route, ...router) => {
            ...
            if (util.isString(route) || util.isArray(route)) {
                if (!router || router.length === 0) {
                    throw new Error('Please supply a router to add');
                } else {
            ...
            // We have functions as the only parameters, attach to the end of ALL routes
                ...
                addGlobalHandlers(createRouteChain('all', ...[route, ...router]));

            // No idea what we have...
            } else {
                throw new Error(`Invalid first parameter ${route} for router.use()`);
            }
        },
