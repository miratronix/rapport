 ...
 *
 * @param {string} verb Verb for the routes.
 * @param {...function} handlers Handler functions to use.
 * @return {Array} Array of handlers (AKA a Route Chain).
 */
...
/**
 * Gets an object of verbs supported by a route chain.
 *
 * @param {object[]} chain The route chain to check.
 * @return {object} An object containing a key for every supported verb with a value of true.
 */
const getHandledVerbs = (chain) => {
    const handledVerbs = {};

    for (let i = 0; i < chain.length; i++) {
        handledVerbs[chain[i].verb] = true;
    }
    return handledVerbs;
};
...
/**
 * Pushes a route chain onto another chain.
 *
...
const Router = () => {
    ...
    /**
     * Contains an array of regex routes objects. These contain the following properties: {route, keys, regex, handledVerbs, chain}.
     */
    const regexRoutes = [];
    ...
    /**
     * Adds the specified chain to all existing routes.
     *
    ...
    const addToAllRoutes = (chain) => {
        ...
        for (let i = 0; i < regexRoutes.length; i++) {
            ...
            regexRoutes[i].handledVerbs = getHandledVerbs(regexRoutes[i].chain);
        }
    };
    ...
    const addRoute = (route, chain) => {
        ...
        if (isRegexRoute(route)) {
            ...
            // Add chain to existing route if it's there and update the handled verbs for the route
            for (let i = 0; i < regexRoutes.length; i++) {
            ...
            const fullChain = appendRouteChain([], chain);
            const handledVerbs = getHandledVerbs(fullChain);

            // Push on a brand new regex route
            regexRoutes.push({ route, keys, regex, handledVerbs, chain: fullChain });
            ...
            appendRouteChain(routes[cleanRoute], chain);
        }
    };
    ...
    const router = {
        ...
        /**
         * Adds a router to this router, or adds a global function.
         *
        ...
        /**
         * Adds a series of handlers for a route covering all HTTP verbs.
         *
         ...
         */
        handle: (req, res) => {
            const route = util.trimSlashes(req.route);
            ...
            const handleRegexRoutes = () => {
                ...
                for (let i = 0; i < routes.length; i++) {
                    ...
                    if (matches !== null) {
                        ...
                        // Make sure the route supports the verb
                        if (routeInfo.handledVerbs.all || routeInfo.handledVerbs[req.verb]) {
