...
module.exports = (wrappedSocket, requestCache, options, body, timeout, cb) => {
    ...
    const wrappedRequest = {
        _req: options.generateRequestId(),
        _b: body
    };

    if (timeout) {
        setTimeout(() => {
            requestCache.reject(wrappedRequest._req, new Error(`Timed out after ${timeout} ms`));
        }, timeout);
    }

    if (cb) {
        requestCache.addCallback(wrappedRequest._req, cb);
        wrappedSocket.send(wrappedRequest);

    } else if (options.Promise) {
        return new options.Promise((resolve, reject) => {
            requestCache.addPromise(wrappedRequest._req, resolve, reject);
            wrappedSocket.send(wrappedRequest);
        });
