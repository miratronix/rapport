 ...
 */
const onMessage = (standardSocket, wrappedSocket, options, handler) => {
    standardSocket.onMessage((msg) => {
        ...
        if (message.responseId) {
            onMessage.handleResponse(message.responseId, message.body, message.error);
 ...
 */
onMessage.handleResponse = (responseId, response, error) => {
    if (response) {
