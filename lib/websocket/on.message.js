...
const onMessage = (standardSocket, wrappedSocket, requestCache, options, handler) => {
    standardSocket.onMessage((msg) => {
        ...
        if (message._res) {
            onMessage.handleResponse(standardSocket, wrappedSocket, requestCache, options, message._res, message._b, message._e);
        } else if (message._req) {
            onMessage.handleRequest(standardSocket, wrappedSocket, requestCache, options, message._req, message._b, handler);
        } else {
