...
const executeWebsocketPlugin = (ws, options, ...plugins) => {
    for (let i = 0; i < plugins.length; i++) {
        if (util.isFunction(plugins[i].extendWebsocket)) {
            plugins[i].extendWebsocket(ws, options);
        }
    }
};
...
const createInstance = (wsImplementation, options) => {
    const rapport = {
        ...
        wrap: (websocket, options) => {
            const opts = createOptions(options, rapport.options);
            const wrappedSocket = wrapSocket(standardizeSocket(websocket), opts);

            // Execute plugins that add to the websocket object
            executeWebsocketPlugin(wrappedSocket, opts, ...rapport.plugins);
